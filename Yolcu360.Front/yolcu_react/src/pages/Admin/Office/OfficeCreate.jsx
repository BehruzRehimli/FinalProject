...
const OfficeCreate = () => {
    ...
    useEffect(() => {
        const getEntity = async () => {
            ...
            try {
                var datas = await axios.get("https://localhost:7079/api/Cities", { headers: { "Authorization": headerToken } })
            } catch (error) {
                console.log(error);
    ...
    const BtnHandler = async (e) => {
        ...
        var headerToken = `Bearer ${adminToken}`
        try {
            var data = await axios.post("https://localhost:7079/api/offices", postValues, {
                headers: {
        ...
        <div className='admin-container'>
            ...
            <Formik>
                <Formik initialValues={{
                    ...
                    // var headerToken = `Bearer ${adminToken}`
                    // try {
                    //     var result = await axios.post("https://localhost:7079/api/offices", values, { headers: { "Authorization": headerToken } })
                    // } catch (error) {
