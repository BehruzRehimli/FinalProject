...
const Admin = () => {
    ...
    useEffect(() => {
        const getCountries = async () => {
            ...
            try {
                var datas = await axios.get(`https://localhost:7079/api/Admins`, { headers: { "Authorization": token } })
                                        ...
                                        <tr key={x.id}>
                                            ...
                                            <td><span className="text-success">
                                                ...
                                                <button onClick={async () => {
                                                    var token = "Bearer " + adminToken
                                                    try {
                                                        var datas = await axios.delete(`https://localhost:7079/api/Admins/${x.id}`, { headers: { "Authorization": token } })
                                                        var datas = await axios.get("https://localhost:7079/api/Admins", { headers: { "Authorization": token } })
                                                        setCountries(prev => { return { ...prev, countries: datas.data, isLoad: true } })
